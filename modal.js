...
// close modal fonctionnalité au bouton (x)
function closeModal() {
...
function validInputs() {
  let valid = true; 
//regex pour le nom , prenom , email et date‡
...
// regex date de naissance 
  ...
  if (!regexName.test(firstNameValue)) {
    setErrorFor(firstNameUser, 'Veuillez entrer votre Prénom correctement !.');
    valid = false;
  } else {
  ...
  if (!regexName.test(lastNameValue)) {
    setErrorFor(lastNameUser, 'Veuillez entrer votre Nom correctement !.');
    valid = false;
  } else {
...
function age (date) {
  ...
  // Enfin, il vérifie si la case est cochée. Sinon, une erreur est définie.
  // La fonction renvoie  valid (valeur), indiquant si toutes les validations ont réussi.
...
// Il récupère tous les boutons radio
// Vérifie si au moins un bouton radio d'emplacement est sélectionné.
...
// Définit un message d'erreur pour un champ de formulaire spécifique.
...
// Supprime tout message d'erreur pour un champ de formulaire spécifique.
...
//   Si validInputs renvoie false, la soumission du formulaire est interrompue ( return;).
 // Sinon, le formulaire est réinitialisé ( form.reset()) pour effacer toutes les données saisies.
